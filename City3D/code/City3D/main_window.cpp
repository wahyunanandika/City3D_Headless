...
void MainWindow::createActions() {
	...
	actionSegmentation->setShortcut(QKeySequence("Ctrl+1"));

	connect(actionExtractRoofs, SIGNAL(triggered()), mainCanvas_, SLOT(extractRoofs()));
	actionExtractRoofs->setShortcut(QKeySequence("Ctrl+2"));

	connect(actionReconstruction, SIGNAL(triggered()), mainCanvas_, SLOT(reconstruct()));
	actionReconstruction->setShortcut(QKeySequence("Ctrl+3"));
...
void MainWindow::createToolBar()
{
	...
	actionOpenPointCloud->setShortcut(QKeySequence("Ctrl+O"));
	...
	actionOpenFootPrint_->setShortcut(QKeySequence("Ctrl+F"));
	...
	actionSaveReconstruction->setShortcut(QKeySequence("Ctrl+S"));
...
{
	curDataDirectory_ = fileName.left(fileName.lastIndexOf("/") + 1);
...
// ============================================================
// HEADLESS MODE — dipanggil dari main.cpp kalau ada argumen
// City3D.exe <pointcloud> <footprint> <output>
// Tidak ada popup, tidak ada dialog — langsung proses dan save
// ============================================================
void MainWindow::runHeadless(const QString& pcPath, const QString& fpPath, const QString& outPath)
{
	Logger::out("-") << "=== HEADLESS MODE ===" << std::endl;
	Logger::out("-") << "PC       : " << pcPath.toStdString() << std::endl;
	Logger::out("-") << "Footprint: " << fpPath.toStdString() << std::endl;
	Logger::out("-") << "Output   : " << outPath.toStdString() << std::endl;

	// Step 1: Load point cloud
	Logger::out("-") << "Loading point cloud..." << std::endl;
	if (!doOpen(pcPath)) {
		Logger::err("-") << "Failed to load point cloud" << std::endl;
		writeStatusFile(outPath, "error: failed to load point cloud");
		return;
	}

	// Step 2: Load footprint
	Logger::out("-") << "Loading footprint..." << std::endl;
	if (!doOpen(fpPath)) {
		Logger::err("-") << "Failed to load footprint" << std::endl;
		writeStatusFile(outPath, "error: failed to load footprint");
		return;
	}

	// Step 3: Segmentation
	Logger::out("-") << "Running segmentation..." << std::endl;
	mainCanvas_->segmentation();

	// Step 4: Extract roofs
	Logger::out("-") << "Extracting roofs..." << std::endl;
	mainCanvas_->extractRoofs();

	// Step 5: Reconstruction
	Logger::out("-") << "Running reconstruction..." << std::endl;
	mainCanvas_->reconstruct();

	// Step 6: Save langsung ke outPath tanpa dialog
	Map* reconstruction = canvas()->reconstruction();
	if (!reconstruction) {
		Logger::err("-") << "Reconstruction failed" << std::endl;
		writeStatusFile(outPath, "error: reconstruction failed");
		return;
	}

	Logger::out("-") << "Saving to: " << outPath.toStdString() << std::endl;
	bool success = MapIO::save(outPath.toStdString(), reconstruction);
	if (success) {
		Logger::out("-") << "Saved successfully." << std::endl;
		writeStatusFile(outPath, "done");
	} else {
		Logger::err("-") << "Failed to save." << std::endl;
		writeStatusFile(outPath, "error: failed to save");
	}
}


void MainWindow::writeStatusFile(const QString& outPath, const std::string& status)
{
	// Tulis status ke file <outPath>.status.txt
	// Script Python polling file ini untuk tahu kapan selesai
	QString statusPath = outPath + ".status.txt";
	QFile f(statusPath);
	if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
		f.write(status.c_str());
		f.close();
	}
}
